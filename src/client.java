                    ...
                    for (int k4 = 0; k4 < 13; k4++) {
                        for (int j6 = 0; j6 < 13; j6++) {
                            ...
                            if (l7 != -1) {
                                ...
                                for (int l11 = 0; l11 < regionIds.length; l11++) {
                    ...
                    for (int j8 = 0; j8 < 13; j8++) {
                        for (int j9 = 0; j9 < 13; j9++) {
                            ...
                            if (i10 != -1) {
                                ...
                                for (int k12 = 0; k12 < regionIds.length; k12++) {
...
//	notifyItemSpawn(item, i + baseX, j + baseY);
                            ...
                            if (k >= j3 && i1 >= k3 && k < j3 + 32 && i1 < k3 + 32) {
                                ...
                                if (class9_1.getInv()[k2] > 0) {
                                    ...
                                    if (itemSelected == 1 && class9_1.isInventoryInterface()) {
                                        ...
                                        menuActionName[menuActionRow] = "Examine @lre@" + itemDef.name + " @gre@(@whi@" + (class9_1.getInv()[k2] - 1) + ", " + class9_1.getId() + "@gre@)";
    ...
    private void drawMenu() {
        ...
        int color = 6116423;
        DrawingArea.drawPixels(height, offsetY, offsetX, color, width);
        DrawingArea.drawPixels(16, offsetY + 1, offsetX + 1, 0, width - 2);
        DrawingArea.fillPixels(offsetX + 1, width - 2, height - 19, 0, offsetY + 18);
        chatTextDrawingArea.method385(color, "Choose Option", offsetY + 14, offsetX + 3);
        int j1 = super.mouseX;
        int k1 = super.mouseY;
        if (menuScreenArea == 0) {
            j1 -= 4;
        ...
        for (int l1 = 0; l1 < menuActionRow; l1++) {
            int i2 = offsetY + 31 + (menuActionRow - 1 - l1) * 15;
            int j2 = 0xffffff;
            if (j1 > offsetX && j1 < offsetX + width && k1 > i2 - 13 && k1 < i2 + 3)
                j2 = 0xffff00;
            chatTextDrawingArea.method389(true, offsetX + 3, j2, menuActionName[l1], i2);
        }
    }
    ...
    private int method54() {
        for (int i = 0; i < terrianData.length; i++) {
            if (terrianData[i] == null && terrianIndices[i] != -1)
                return -1;
            if (objectData[i] == null && objectIndices[i] != -1)
                return -2;
        }
        boolean flag = true;
        for (int j = 0; j < terrianData.length; j++) {
        ...
        do {
            ...
            do {
                ...
                if (onDemandData.dataType == 3 && loadingStage == 1) {
                    for (int i = 0; i < terrianData.length; i++) {
                        if (terrianIndices[i] == onDemandData.ID) {
                            terrianData[i] = onDemandData.buffer;
        ...
        for (int k = 0; k < Model.anInt1687; k++) {
            ...
            if (k1 == 2 && worldController.method304(plane, i1, j1, l) >= 0) {
                ...
                if (itemSelected == 1) {
                    ...
                    menuActionName[menuActionRow] = "Examine @cya@" + objectDef.name + " @gre@ (@whi@" + objectDef.type + ", " + i1 + " , " + j1 + "@gre@)";
                ...
                if (class19 != null) {
                    for (Item item = (Item) class19.getFirst(); item != null; item = (Item) class19.getNext()) {
                        ...
                        if (itemSelected == 1) {
                            ...
                            menuActionName[menuActionRow] = "Examine @lre@" + itemDef.name + " @gre@(@whi@" + item.ID + ", " + i1 + ", " + j1 + "@gre@)";
...
//	entityMessage(npc);
    ...
    private boolean interfaceIsSelected(RSInterface class9) {
        ...
        for (int i = 0; i < class9.getAnIntArray245().length; i++) {
            ...
            if (class9.getAnIntArray245()[i] == 2) {
                if (j >= k)
                    return false;
            } else if (class9.getAnIntArray245()[i] == 3) {
                if (j <= k)
                    return false;
            } else if (class9.getAnIntArray245()[i] == 4) {
                if (j == k)
                    return false;
            } else if (j != k)
                return false;
    ...
    private DataInputStream openJagGrabInputStream(String s)
        ...
        JagSocket = openSocket(43595);
        JagSocket.setSoTimeout(10000);
        java.io.InputStream inputstream = JagSocket.getInputStream();
        OutputStream outputstream = JagSocket.getOutputStream();
            ...
            switch (pktType) {
                ...
                case 241:
                    ...
                    if (pktType == 73) {
                        ...
                        terrianData = new byte[k16][];
                        objectData = new byte[k16][];
                        regionIds = new int[k16];
                        terrianIndices = new int[k16];
                    ...
                    if (pktType == 241) {
                        ...
                        terrianData = new byte[l16][];
                        objectData = new byte[l16][];
                        regionIds = new int[l16];
                        terrianIndices = new int[l16];
