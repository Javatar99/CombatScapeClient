...
public final class ItemDef {
    ...
    public static void unpackConfig(StreamLoader streamLoader) throws IOException {
        ...
        //stream = new Stream(Files.readAllBytes(new File(Signlink.findcachedir() + "OBJ.DAT").toPath()));
        //Stream stream = new Stream(Files.readAllBytes(new File(Signlink.findcachedir() + "OBJ.IDX").toPath()));
    ...
    private static void dumpModelIds() {
        StringBuilder modelDump = new StringBuilder();
        StringBuilder dumpItems = new StringBuilder();
        for (int i = 0; i < totalItems; i++) {
            ItemDef def = forID(i);
            if (def.modelID > 14926) {
                modelDump.append("ID: ").append(def.id).append(" Model ID: ").append(def.modelID).append("\n");
            }
            dumpItems.append("Name: ").append(def.name).append("\n");
            dumpItems.append("ID: ").append(def.id).append("\n");
            dumpItems.append("Model ID: ").append(def.modelID).append("\n");
            dumpItems.append("actions: ").append(Arrays.toString(def.actions)).append("\n").append("\n\n");
        }
        try {
            Files.write(new File(Signlink.findcachedir() + "/modelIdDump.txt").toPath(),
                    modelDump.toString().getBytes());
        } catch (IOException e) {
            e.printStackTrace();
        }
        try {
            Files.write(new File(Signlink.findcachedir() + "/itemDataDump.txt").toPath(),
                    dumpItems.toString().getBytes());
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
